...
def train():
    ...
    train = smax_data_tuples[:300] + ithack_data_tuples[:300]
    test = smax_data_tuples[800:] + ithack_data_tuples[800:]
    del smax_data_tuples
    del ithack_data_tuples
    del livedoor
    ...
    for j in range(100):
        ...
        for i in range(0, n, bs):
            x = Variable(xtrain[sffindx[i:(i+bs)]])
            y = Variable(ytrain[sffindx[i:(i+bs)]])
            model.cleargrads()
            loss = model(x, y)
            loss.backward()
            optimizer.update()
